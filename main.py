...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    search_results = [AStar(heuristic_type, weight, max_nr_states_to_expand).solve_problem(problem) for weight in
                      weights]
...
def mda_problem_with_astar_experiments():
    print()
    print(
        'Solve the MDA problem (moderate input, only distance objective, A*, MaxAirDist & SumAirDist & MSTAirDist heuristics).')
...
def mda_problem_anytime_astar_experiments():
    ...
    anytime_anywhere = AnytimeAStar(MDAMSTAirDistHeuristic,
                                    max_nr_states_to_expand_per_iteration=150)
    res = anytime_anywhere.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
